...
export default function RegisterPage() {
  ...
  const roles = [
    ...
    { value: "admin",    en: "Admin",    hi: "प्रशासक" },
  ...
  return (
    <main className="min-h-screen bg-parchment dark:bg-[#1c1814] flex items-center justify-center px-4 py-16 relative">
      ...
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        <div className="absolute -top-32 -left-32 w-[500px] h-[500px] rounded-full bg-saffron/10 blur-[120px]" />
        <div className="absolute -bottom-32 -right-32 w-[400px] h-[400px] rounded-full bg-gold/10 blur-[100px]" />
      ...
      <div className="relative w-full max-w-md">
        ...
        >
          ...
          <h1 className="font-display text-2xl font-bold text-charcoal dark:text-white/90">Join HLAD</h1>
          <p className="font-body text-sm text-charcoal-muted mt-1 font-hindi">हिंदी साहित्य परिवार में आपका स्वागत है</p>
        </motion.div>
        ...
        >
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="font-body text-sm font-medium text-charcoal" htmlFor="name">
                Full name
              </label>
              <input
                id="name"
                ...
                {roles.map((r) => (
                  <button
                    ...
                    className={`rounded-xl border px-3 py-2.5 text-left transition-all ${
                      form.role === r.value
                        ? "border-saffron bg-saffron/8 text-saffron"
                        : "border-charcoal/12 bg-parchment/40 text-charcoal-muted hover:border-charcoal/25"
                    }`}
                  >
                    <span className="font-body block text-sm font-semibold">{r.en}</span>
                    <span className="font-hindi block text-xs opacity-80">{r.hi}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Confirm Password */}
            <div className="flex flex-col gap-1.5">
              <label className="font-body text-sm font-medium text-charcoal" htmlFor="confirm">
                Confirm password
              </label>
              <input
                id="confirm"
        ...
        >
          ...
          <Link href="/login" className="font-semibold text-saffron hover:text-saffron-deep transition-colors no-underline">
            Sign in
          </Link>
        </motion.p>
